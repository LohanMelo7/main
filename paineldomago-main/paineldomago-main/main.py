...
class metodos:
    def whois():
        ...
        match rsp.status_code:
            case 200:
                metodos.ndict(rspj)
                print("%s%sPeço desculpas por não traduzir as respostas, veja porque não traduzir em: https://github.com/LohanMelo7"%(start,Y,r))
...
def menu():
    ...
    while True:
        ...
        cls()
        print(" ╭"+"─"*(size)+"╮")
        print(" │"+str("PainelDoMago").center(size)+"│")
        print(" ╞"+"═"*(size)+"╡")
        _menu()
        print(" │","│".rjust(size))
